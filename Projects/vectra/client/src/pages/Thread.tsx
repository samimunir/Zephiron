...
export default function ThreadPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setErr(e?.response?.data?.message || "Failed to load thread.");
      } finally {
  ...
  async function loadMore() {
    ...
    try {
      ...
      alert(e?.response?.data?.message || "Failed to load more messages.");
    }
  }

  async function submit() {
    if (!id || !body.trim() || !canPost) return;
    setPosting(true);
    try {
      const { message } = await postMessage(id, body.trim());
      setMessages((prev) => [...prev, message]);
  ...
  function replaceMessage(updated: DiscussionMessage) {
    setMessages((prev) =>
  ...
  return (
    <AppShell>
      ...
      {err && !thread && (
        <div className="rounded-xl border border-red-300/50 bg-red-50 p-4 text-sm text-red-700 dark:border-red-800/50 dark:bg-red-900/20 dark:text-red-200">
          {err}
        </div>
      )}
...
function MessageBubble({
  ...
  m: DiscussionMessage;
  mine: boolean;
  onSave: (m: DiscussionMessage) => void;
  ...
  async function save() {
    ...
    try {
      const { message } = await editMessageApi(m._id, val.trim());
      setVal(message.body);
      onSave(message); // update in parent list
      setEditing(false);
    } catch (e: any) {
      alert(e?.response?.data?.message || "Failed to save.");
    } finally {
  ...
  async function remove() {
    ...
    try {
      await deleteMessageApi(m._id);
      onDelete(m._id); // soft-delete in parent list
    } catch (e: any) {
      alert(e?.response?.data?.message || "Failed to delete.");
    }
  }
